  ...
  if (role === 'MERCHANT') {
    ...
    panelContent.innerHTML = `
      <div class="role-widget" style="padding: 10px 0;">
        ...
        <p style="font-size: 12px; color: var(--text-secondary); line-height: 1.4;">Buat invoice dan kelola transaksi kasir melalui aplikasi WarungPOS.</p>
      </div>
    `;
...
async function loadLoansList() {
  ...
  try {
    const res = await fetch(`${BASE_URL}/api/v1/loans/me`, {
      headers: { 'Authorization': `Bearer ${token}` }
    });
    const result = await res.json();
    if (!res.ok) throw new Error(result.error?.message || 'Gagal memuat pinjaman');
    const myLoans = result.data?.loans || [];
    ...
    myLoans.forEach(loan => {
      const progressPercent = loan.total_due > 0 ? Math.min(100, Math.round((loan.paid_amount / loan.total_due) * 100)) : 0;
      ...
      item.innerHTML = `
        ...
        <div class="loan-metric-grid">
          ...
          <div class="loan-metric">
            <span>Bunga (10%)</span>
            <span>Rp ${loan.interest_amount.toLocaleString('id-ID')}</span>
          </div>
          <div class="loan-metric">
            <span>Sudah Dibayar</span>
            <span>Rp ${loan.paid_amount.toLocaleString('id-ID')}</span>
          </div>
          <div class="loan-metric">
            <span>Sisa Tagihan</span>
            <span class="text-danger" style="font-weight: 700;">Rp ${loan.remaining.toLocaleString('id-ID')}</span>
          </div>
        </div>
        ...
        <button class="btn btn-success btn-small btn-repay-trigger" onclick="selectLoanForRepay('${loan.id}', ${loan.remaining})" style="width: 100%; font-size: 12px; padding: 6px 12px; margin-top: 6px;">

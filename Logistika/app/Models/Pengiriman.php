...
class Pengiriman {
    ...
    public function triggerWebhookCallback($resi, $status, $ext_order_id) {
        ...
        $callbackUrl = getenv('INTEGRATION_WEBHOOK_CALLBACK_URL') ?: '';
        $secret = getenv('INTEGRATION_WEBHOOK_SECRET') ?: '';
        if ($callbackUrl === '' || strlen($secret) < 32) return;

        $eventId = 'shipment-status-' . $resi . '-' . time();
        $payload = [
            'app' => 'LogistiKita',
            'event' => 'shipment.status_changed',
            'event_id' => $eventId,
            'order_id' => (int)$ext_order_id,
            'resi' => $resi,
            'tracking_reference' => $resi,
        ...
        curl_setopt($ch, CURLOPT_TIMEOUT, 5); // Timeout cepat agar tidak memblokir thread

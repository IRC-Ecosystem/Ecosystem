...
exports.setActive = async ({ id, provider }) => {
  const connection = await new Promise((resolve, reject) => {
    db.getConnection((error, leased) => error ? reject(error) : resolve(leased));
  });
  try {
    ...
    await new Promise((resolve, reject) => connection.query(
      "UPDATE api_integrations SET is_active = 0 WHERE provider = ?",
      [provider],
      (error) => error ? reject(error) : resolve()
    ));
    const result = await new Promise((resolve, reject) => connection.query(
      "UPDATE api_integrations SET is_active = 1 WHERE id = ? AND provider = ?",
      [id, provider],
      (error, updateResult) => error ? reject(error) : resolve(updateResult)
    ));

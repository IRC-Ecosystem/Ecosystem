...
const seedProducts = async (connection) => {
  for (const product of products) {
    await connection.execute(
      ...
      `,
      [product[0], product[1], product[2], product[2], product[2], product[3], product[4], product[0]]
    );
  }
};
...
const seedTransactions = async (connection) => {
  ...
  try {
    ...
    for (const item of items) {
      ...
      await connection.execute(
        "UPDATE products SET on_hand = GREATEST(on_hand - ?, 0), available = on_hand - reserved, stock = on_hand - reserved WHERE id = ?",

...
Pada alur penjualan langsung, kasir memilih produk dan quantity untuk membuat invoice `pending_payment`. Setelah pembayaran SmartBank berhasil, transaksi berubah menjadi `paid` dan stok produk otomatis berkurang.
...
5. **SmartBank melalui Connector resmi**
   Pembayaran aktif diarahkan ke SmartBank Connector. API key disimpan di backend POS, konsumen mengotorisasi pembayaran dengan PIN SmartBank, dan sistem menyimpan hasil pembayaran ke tabel `payments`.
...
- **Proses:** sistem memvalidasi status transaksi, mengecek ulang stok, memproses pembayaran SmartBank melalui Connector resmi, menyimpan record `payments`, lalu mengubah status menjadi `paid` jika berhasil.
...
WarungPOS menggunakan database MySQL. Tabel inti transaksi tetap terdiri dari `users`, `products`, `transactions`, dan `transaction_items`, lalu versi sekarang menambahkan tabel pendukung untuk payment, refund, integrasi, outbox event, dan reconciliation.
...
| `status` | ENUM | `pending`, `pending_payment`, `approved`, `paid`, `rejected`, `refunded`. |
...
### 5.5 Tabel Pendukung Versi Sekarang

| Tabel | Fungsi |
| --- | --- |
| `payments` | Menyimpan record pembayaran terpisah dari transaksi, termasuk provider, status, amount, payment request ID, provider reference, idempotency key, response code/body, kasir, dan waktu bayar. |
| `refunds` | Menyimpan intent dan hasil reversal/refund SmartBank. |
| `api_integrations` | Menyimpan konfigurasi endpoint eksternal untuk API Integrator. |
| `outbox_events` | Menyimpan event penting seperti invoice paid agar integrasi eksternal bisa diproses oleh worker. |
| `payment_reconciliation` | Menyimpan hasil pengecekan kesesuaian status payment lokal dengan provider. |

Detail schema paling aktual ada di:
...
### 5.7 Sintaks SQL Database

Contoh ringkas berikut menunjukkan tabel inti aplikasi. Untuk setup penuh versi sekarang, gunakan file migration karena di sana juga ada tabel `payments`, `refunds`, `api_integrations`, `outbox_events`, dan `payment_reconciliation`.
...
| GET | `/manager/payments` | Riwayat dan ringkasan payment. |
| POST | `/manager/payments/:id/refund` | Refund/reversal payment SmartBank. |
| POST | `/manager/transactions/:id/void` | Void transaksi belum dibayar oleh manager dengan alasan wajib. |
| GET | `/manager/audit-logs` | Melihat 100 audit log terbaru. |
| GET | `/manager/smartbank-wallet` | Halaman linkage wallet toko SmartBank. |
| POST | `/manager/smartbank-wallet/otp/request` | Meminta OTP linkage wallet toko. |
| POST | `/manager/smartbank-wallet/otp/verify` | Verifikasi OTP linkage wallet toko. |
| POST | `/manager/smartbank-wallet/link` | Menautkan wallet toko ke external ID POS. |
| GET | `/manager/api-integrator` | Halaman API Integrator. |
| GET | `/manager/api-integrator/local` | Dokumentasi endpoint lokal POS. |
| GET | `/manager/api-integrator/:provider` | Kelola endpoint provider eksternal. |
| POST | `/manager/api-integrator/:provider/endpoints` | Tambah endpoint eksternal. |
| POST | `/manager/api-integrator/:provider/endpoints/:id/update` | Update endpoint eksternal. |
| POST | `/manager/api-integrator/:provider/endpoints/:id/test` | Test endpoint eksternal. |
| POST | `/manager/api-integrator/:provider/endpoints/:id/activate` | Aktifkan endpoint eksternal yang sudah OK. |
| POST | `/manager/api-integrator/:provider/endpoints/:id/delete` | Hapus endpoint eksternal. |
| GET | `/manager/api-spec.json` | Export spesifikasi API lokal. |
| GET | `/manager/api-health.json` | Health check API/integrasi dari sisi manager. |
...
```txt
http://localhost:3002/manager/api-integrator
```

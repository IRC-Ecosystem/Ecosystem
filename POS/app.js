...
const { startBackgroundWorkers } = require("./services/backgroundWorkers");

const app = express();
const PORT = process.env.PORT || 3000;
const SESSION_MAX_AGE = 30 * 60 * 1000;
const SESSION_SECRET = process.env.SESSION_SECRET || (process.env.NODE_ENV === "test" ? "test-session-secret-minimum-32-characters" : "");
if (!SESSION_SECRET) {
...
app.get("/health", (req, res) => res.status(200).json({ status: "ok", service: "warungpos" }));

...
### [SEBAGIAN SELESAI] FR-PAY-001 - Sistem harus menyimpan idempotency key

Setiap pembayaran harus memiliki idempotency key yang disimpan di database.

Status saat ini:

- Tabel `payments` sudah memiliki `idempotency_key`.
- Sudah ada unique index untuk mencegah key yang sama dipakai ulang.
- Masih perlu test end-to-end untuk double click dan retry request.
...
### [SEBAGIAN SELESAI] FR-PAY-002 - Sistem harus menolak idempotency key yang sama dengan payload berbeda

Jika idempotency key sudah pernah digunakan, sistem harus membandingkan payload pembayaran.

Status saat ini:

- `payments.request_fingerprint` sudah dipakai untuk membandingkan payload.
- Masih perlu test otomatis untuk memastikan konflik payload ditolak di seluruh flow UI/API.
...
### [SEBAGIAN SELESAI] FR-PAY-003 - Sistem harus memiliki payment state machine

Sistem harus memisahkan status transaksi dan status pembayaran.

Status saat ini:

- Status transaksi dan payment sudah dipisah pada tabel `transactions` dan `payments`.
- Status `refunded` sudah tersedia.
- Status `voided` dan validasi transisi lengkap belum tersedia.
...
### [SELESAI] FR-FIN-001 - Sistem harus mengintegrasikan pembayaran POS dengan SmartBank secara lebih kuat

*Status: Sudah diimplementasikan pada `controllers/kasirController.js` dan berhasil menyimpan hasil pembayaran (termasuk status, payment request ID, dan response) secara jelas ke tabel `payments`.*
...
### [SEBAGIAN SELESAI] FR-AUD-001 - Sistem harus mencatat audit log untuk aksi penting
...
Status saat ini: aksi `transaction.voided` sudah dicatat atomik. Aksi lain pada daftar di atas masih perlu ditambahkan.
...
### [SEBAGIAN SELESAI] FR-VOID-001 - Sistem harus mendukung void transaksi
...
Status saat ini: manager dapat mengirim `POST /manager/transactions/:id/void` dari dashboard untuk status `pending`, `approved`, atau `pending_payment`, dengan alasan wajib. Void dibatasi sebelum settlement (`stock_deducted = 0`), sehingga tidak ada stok yang harus dikembalikan. Transaksi `paid` harus memakai refund.

### [SEBAGIAN SELESAI] FR-VOID-002 - Void harus membutuhkan permission supervisor atau manager
...
Status saat ini: route void dilindungi `requireRole("manager")`. Percobaan void yang berhasil dicatat; pencatatan percobaan gagal dan role supervisor granular masih perlu dibuat.

### [SEBAGIAN SELESAI] FR-REF-001 - Sistem harus mendukung refund penuh

Sistem harus mendukung pengembalian dana penuh untuk transaksi yang valid.

Status saat ini:

- Tabel `refunds`, service refund, dan route manager refund sudah tersedia untuk SmartBank reversal.
- Masih perlu melengkapi efek refund ke status transaksi, stok, audit log, dan ledger.
...
### [SEBAGIAN SELESAI] FR-EVT-001 - Sistem harus memiliki transactional outbox

Setiap event penting harus disimpan di outbox bersama transaksi database.

Status saat ini:

- Tabel `outbox_events` sudah tersedia.
- Event `invoice.paid` dan `umkm.insight.invoice_paid` sudah dibuat saat transaksi berhasil paid.
- Worker outbox dasar sudah tersedia.
- Masih perlu cakupan event lengkap untuk approved, rejected, voided, refunded, dan stock event.
...
### [SEBAGIAN SELESAI] FR-OPS-001 - Sistem harus memiliki health endpoint

Sistem harus menyediakan endpoint health untuk mengecek aplikasi aktif.

Status saat ini:

- Endpoint `/health` sudah tersedia dan mengembalikan status aplikasi.
- Belum mengecek dependency seperti database, session store, SmartBank Connector, dan worker.
...
1. FR-AUD-001 - Sistem harus mencatat audit log untuk aksi penting.
2. FR-VOID-001 - Sistem harus mendukung void transaksi.
3. FR-TST-001 - Sistem harus memiliki test untuk pembayaran ganda.
4. FR-TST-003 - Sistem harus memiliki test untuk transaksi paralel.

...
const { validateMoney } = require("../services/paymentPolicy");
...
exports.checkout = async (req, res) => {
  ...
  try {
    ...
    const result = await TransactionModel.createTransactionWithItems({
      transaction: {
        invoice,
    ...
    if (!result.success) {
      setFlash(req, { type: "error", message: "Stock berubah saat checkout. Silakan periksa cart kembali." });
      return res.redirect(redirectUrl);
    }
...
exports.payWithSmartBank = async (req, res) => {
  ...
  try {
    ...
    const amount = validateMoney(Number(transaction.grand_total));
    const existingPayment = await PaymentModel.getLatestSuccessfulByTransactionId(transaction.id);
    let finalized;
    if (existingPayment?.method === "smartbank") {
      finalized = await TransactionModel.payTransaction({ transactionId: transaction.id, cashierId: null, paymentMethod: "smartbank" });
    } else {
      const idempotencyKey = `warungpos-${transaction.invoice}`;
      ...
      let settled;
      try {
        settled = await SmartBankConnector.pay({
      ...
      finalized = await TransactionModel.settleSmartBankPayment({
        transactionId: transaction.id,
        cashierId: null,
        payment: {
          amount,
          paymentRequestId: settled.payment_request_id || null,

...
const validateProductInput = (product) => {
  ...
  try {
    validateMoney(product.harga);
  } catch (error) {
    return "Harga produk harus berupa bilangan bulat positif.";
  }
...
exports.updateProduct = async (req, res) => {
  ...
  try {
    ...
    if (Number(existingProduct.reserved || 0) > product.stock) {
      setFlash(req, { type: "error", message: "Stok fisik tidak boleh lebih kecil dari stok yang sedang di-reserve.", modal: "edit", productId, oldInput: req.body });
      return res.redirect("/operator");
    }
...
exports.updateStock = async (req, res) => {
  ...
  try {
    ...
    if (Number(existingProduct.reserved || 0) > stock) {
      setFlash(req, { type: "error", message: "Stok fisik tidak boleh lebih kecil dari stok yang sedang di-reserve.", modal: "stock", productId, oldInput: req.body });
      return res.redirect("/operator");
    }

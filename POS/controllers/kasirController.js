...
const { requireConnectorPaymentMethod, validateMoney } = require("../services/paymentPolicy");
...
const PAYMENT_METHODS = ["smartbank"];
const FEE_RATE = 0.01;
...
const processSmartBankEndpoint = async ({ transaction }) => {
  const existing = await PaymentModel.getLatestSuccessfulByTransactionId(transaction.id);
  if (!existing || existing.method !== "smartbank") {
    return { success: false, status: "failed", payment_request_id: null, message: "Konsumen belum mengotorisasi pembayaran dari halaman status pesanannya." };
  }
  return {
    success: true,
    status: "success",
    payment_request_id: existing.payment_request_id,
    message: "Settlement SmartBank konsumen sudah terverifikasi.",
    endpoint_name: "SmartBank Connector",
...
exports.payTransaction = async (req, res) => {
  ...
  if (!PAYMENT_METHODS.includes(paymentMethod)) {
    try {
      requireConnectorPaymentMethod(paymentMethod);
    } catch (error) {
      setFlash(req, { type: "error", message: error.message });
      return res.redirect("/kasir");
    }
  }
  ...
  try {
    ...
    validateMoney(Number(transaction.grand_total));
    requireConnectorPaymentMethod(paymentMethod);

    const existingPayment = await PaymentModel.getLatestSuccessfulByTransactionId(transactionId);
    const smartBankResult = existingPayment?.method === "smartbank"
      ? { success: true, payment_request_id: existingPayment.payment_request_id, endpoint_name: "SmartBank Connector", response_code: existingPayment.response_code, response_body: existingPayment.response_body }
      : { success: false, message: "Konsumen belum mengotorisasi pembayaran SmartBank dari halaman status pesanannya." };
    ...
    setFlash(req, {
      ...
      message: paymentMethod === "smartbank"
        ? "Settlement SmartBank konsumen berhasil diverifikasi melalui Connector."
...
exports.smartBankPayment = async (req, res) => {
  ...
  try {
    ...
    const smartBankResult = await processSmartBankEndpoint({ transaction });

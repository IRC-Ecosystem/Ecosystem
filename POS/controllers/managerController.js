...
exports.refundPayment = async (req, res) => {
  ...
  try {
    const payment = await PaymentModel.getRefundableById(paymentId);
    if (!payment) {
      setFlash(req, { type: "error", message: "Payment tidak ditemukan." });
    } else {
      await processRefund({ payment, reasonCode, store: RefundModel, connector: SmartBankConnector });
      setFlash(req, { type: "success", message: "Refund SmartBank berhasil direversal melalui Connector." });
    }
  } catch (error) {
    setFlash(req, { type: "error", message: error.message || "Refund SmartBank gagal." });
  }
...
exports.voidTransaction = async (req, res) => {
  ...
  if (!transactionId) {
    setFlash(req, { type: "error", message: "Transaksi tidak ditemukan." });
    return res.redirect("/manager");
  }

  if (reason.length < 3 || reason.length > 500) {
    setFlash(req, { type: "error", message: "Alasan void wajib diisi (3-500 karakter)." });
    return res.redirect("/manager");
  }

  try {
    const result = await TransactionModel.voidTransaction({
      transactionId,
    ...
    if (!result.success) {
      const message = result.reason === "not_found"
        ...
        : result.reason === "payment_exists"
          ? "Transaksi sudah memiliki payment sukses dan harus direfund, bukan di-void."
          : "Hanya transaksi yang belum dibayar dan belum mengurangi stok yang dapat di-void.";
      setFlash(req, { type: "error", message });
      return res.redirect("/manager");
    }

    setFlash(req, { type: "success", message: `Transaksi ${result.transaction.invoice} berhasil di-void dan dicatat pada audit log.` });
  } catch (error) {
    console.error("Void transaction error:", error);
    setFlash(req, { type: "error", message: "Gagal melakukan void transaksi." });
  }
...
exports.auditLogs = async (req, res) => {
  try {
    const auditLogs = await AuditLogModel.getRecent({ limit: 100, offset: 0 });
    const normalizedAuditLogs = auditLogs.map((log) => ({
      ...log,
      actor_name: log.user_name || log.user_email || "Sistem",
      created_at_formatted: formatDate(log.created_at),
      before_summary: typeof log.before_data === "string" ? log.before_data : JSON.stringify(log.before_data || {}),
      after_summary: typeof log.after_data === "string" ? log.after_data : JSON.stringify(log.after_data || {})
    }));

    return res.render("manager/audit-logs", {
      pageTitle: "Audit Log",
    ...
    console.error("Manager audit logs error:", error);
    return res.status(500).render("manager/audit-logs", {
      pageTitle: "Audit Log",
      auditLogs: [],
      todayLabel: "",
      dashboardError: "Gagal memuat audit log dari database."
    });
  }
};

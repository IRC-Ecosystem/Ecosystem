...
DB_CONFIG = {
    ...
    "db": os.getenv("DB_NAME", "tugasgateaway"),
}

if not DB_CONFIG["db"].replace("_", "").isalnum():
...
async def init_db():
    ...
    try:
        ...
        async with conn.cursor() as cur:
            ...
            await cur.execute(f"USE `{DB_CONFIG['db']}`")
            ...
            await cur.execute("""
                CREATE TABLE IF NOT EXISTS api_plugins (
                    ...
                    default_endpoint VARCHAR(255) NOT NULL DEFAULT '/',
                    method VARCHAR(10) NOT NULL DEFAULT 'POST',
                    health_endpoint VARCHAR(255) DEFAULT '/health',
                    auth_header_name VARCHAR(120) DEFAULT NULL,
                    ...
                    INDEX idx_api_plugins_active (is_active),
                    INDEX idx_api_plugins_app_id (app_id)
                )
            """)
...
async def get_conn():
    """Ambil koneksi MySQL dari pool aplikasi."""
    global _pool
    if _pool is None:
        async with _pool_lock:
            if _pool is None:
                ...
                _pool = await aiomysql.create_pool(**cfg, autocommit=True, minsize=1, maxsize=10, pool_recycle=300)
    return PooledConnection(_pool, await _pool.acquire())
...
# ==========================================
# FUNGSI LOG RINGKAS LEGACY
# ==========================================

async def catat_legacy_log(user_id: str, endpoint: str, data_tambahan: dict = None):
    ...
    try:
        ...
        async with conn.cursor() as cur:
            ...
            """, (
                ...
                "SUCCESS" if str(smartbank_status).lower() in ("sukses", "success", "ok") else "FAILED"
            ))
        conn.close()
...
async def init_financial_table():
    ...
    try:
        ...
        async with conn.cursor() as cur:
            ...
            # Seed contoh hanya jika diminta eksplisit dari environment.
            await cur.execute("SELECT COUNT(*) as c FROM financial_ledger")
            row = await cur.fetchone()
            if row[0] == 0 and os.getenv("SEED_DEMO_DATA", "false").lower() == "true":
                ...
                await cur.executemany("""
                    ...
                    VALUES (%s, %s, %s, %s, %s, %s)
                """, [
                    (today, 'aset',        'Modal Awal',       'Modal awal pendirian API Gateway',          5000000,  'system'),
                    (today, 'aset',        'Perangkat Keras',  'Pembelian server & perangkat jaringan',     3000000,  'system'),
                    (today, 'pengeluaran', 'Biaya Cloud',      'Sewa VPS / cloud hosting bulanan',          500000,   'system'),
                    (today, 'pengeluaran', 'Biaya Listrik',    'Tagihan listrik kantor bulan ini',          250000,   'system'),
                    (today, 'kewajiban',   'Hutang Vendor',    'Cicilan software lisensi middleware',       1000000,  'system'),
                    (today, 'pemasukan',   'Paket Langganan',  'Pembayaran langganan aplikasi client',      750000,   'system'),
                ])
